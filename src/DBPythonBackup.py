...
class PythonBackup(updg, reload_filesets, file_sizes, db_services, os_services):
    """
    This class contains the methods to Backup an entire server based on the configured
    PostGreSQL database of backup sets
    ...
    def backup_start(self, run_frequency):
        ...
        for index in range(len(backup_list_in)):
            ...
            for key in backup_list_in[index]:  # loop through backup set key fields
                ...
                elif key == "StorageSetID":
                    storage_set_id = backup_list_in[index][key]
                elif key == "FileSetID":
                    file_set_id = backup_list_in[index][key]
                elif key == "FrequencyID":
                    frequency_id = str(backup_list_in[index][key])
                    frequency = self.frequency_getter(frequency_id)
                    if run_frequency.upper() == str(frequency).upper():
                        skipping = False
                elif key == "Versions":
            ...
            try:
                ...
                storage_path = self.storage_path_getter(storage_set_name)
                if not os.path.exists(storage_path):
                    storage_path = self.storage_path_getter(storage_path)
                ...
                include_files_list = self.fileset_includes_getter(file_set_name)
                exclude_files_list = self.fileset_excludes_getter(file_set_name)
                recurse = self.fileset_recurse_getter(file_set_name)
    ...
    def backupSetGetter(self):
        ...
        :return: Nothing
        """
        self.BackupSet_AoD, self.StorageSet_AoD, self.FileSet_AoD = db_services.dbSetsCollect()

    def frequency_getter(self, frequency_needle):
        frequenciesIn = self.FileSet_AoD

        for index in range(len(frequenciesIn)):
            for key in frequenciesIn[index]:
                if key == "FrequencyID" and frequenciesIn[index]['FrequencyID'] == frequency_needle:
                    return frequenciesIn[index]["Frequency"]

    def storage_path_getter(self, storageSet_needle):
        storageSetsIn = self.StorageSet_AoD

        for index in range(len(storageSetsIn)):
            for key in storageSetsIn[index]:
                if key == "StorageSetName" and storageSetsIn[index]['StorageSetName'] == storageSet_needle:
                    return storageSetsIn[index]["StoragePath"]
